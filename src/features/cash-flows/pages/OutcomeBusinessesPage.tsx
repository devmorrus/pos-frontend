...
export default function OutcomeBusinessesPage() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        setError(getErrorMessage(requestError, "Gagal memuat daftar pengeluaran toko."));
      } finally {
        setIsLoading(false);
      }
    }
  ...
  return (
    <ProtectedPageShell
      ...
      description="Catat dan pantau transaksi pengeluaran toko manual yang langsung diposting ke jurnal keuangan."
    >
      ...
      <AppTableShell
        ...
        description="Filter transaksi pengeluaran per tanggal, outlet, akun, dan catatan."
        actions={
          <Link
            to="/outcome-businesses/create"
      ...
      >
        ...
        {isLoading ? (
          <div className="p-6">
            <AppLoader label="Memuat daftar pengeluaran..." />
          </div>
        ) : items.length === 0 ? (
          <div className="p-6">
            <PagePlaceholder
              title="Belum ada pengeluaran"
              description="Belum ada transaksi pengeluaran toko pada filter yang dipilih."
              status="Data kosong"
            />
          </div>
        ) : (
